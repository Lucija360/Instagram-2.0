...
import {modalState} from "../atoms/modalAtom"; // enables the recoil root
...
function Header() {
 ...
 {/* Middle --Search input field */}
           ...
           <div className="flex items-center justify-end space-x-4">
                <HomeIcon onClick={() => router.push('/')} className="navBtn" />  {/* navBtn is defined in global.css and styles and animates buttons */} 
...
<div className="relative navBtn">
     ...
     <div className="absolute -top-1 -right-2 text-xs w-5 h-5 bg-red-500 rounded-full flex items-center justify-center animate-pulse text-white"> {/* Notification circle */}  
         3 {/* Amount of notifications received */}  
                ...
                <PlusCircleIcon onClick={()=>setOpen(true)} className="navBtn" />  {/* enables the button to function as upload functionality */} 
                <UserGroupIcon className="navBtn" />
                <HeartIcon className="navBtn" />
                <img onClick={signOut}
                src={session.user.image}
                alt='profile pic' 
                className="h-10 w-10 rounded-full cursor-pointer"/> {/* Makes the your avatar logo circle */} 
                 </>
): (
